...
export class Ejecucion {
  ...
  */ 
  ...
  recorrerXML(nodo: any): any {
    ...
    if(this.soyNodo('OBJETO', nodo)){
      ...
      let NuevaEtiqueta = new Etiqueta(id, atributos, hijoEtiqueta, textoo, poss);
  ...
  resolverExpresion(nodo: any,entorno:Entorno): Expresion{
    if (this.soyNodo('VAL', nodo)) {
      ...
      var sim=entorno.buscarVariable(id_,nodo.linea);
      if (sim!=null){
        return new Expresion({tipo_:sim.tipo,valor_:sim.valor});
      }
    }
        ...
        if (sim.tipo==6){
          ...
          if (indice.tipo==1){
            if ((indice.valor>0)&&(indice.valor<=sim.valor.length)){
              return new Expresion({tipo_:TIPO_DATO.NUMBER,valor_:sim.valor[indice.valor-1]});
            }
          }
          Errores.getInstance().push(
            new Error({
    ...
    //DECLARA_ARREGLO
      ...
      //si ambos son numeros
        if (valor1.valor<=valor2.valor){
          ...
          return new Expresion({tipo_:TIPO_DATO.ARREGLO,valor_:nuevoArray});
    ...
    //To_STRING
      ...
      let resultado=<string>valor1.valor;
          return new Expresion({tipo_:TIPO_DATO.STRING,valor_:resultado});
      }
      Errores.getInstance().push(
        new Error({
          tipo: "semantico",
    ...
    //To number
      ...
      //Si no es error, se ejecuta
        ...
        if (valor1.tipo==3){
          if (valor1.valor){
            return new Expresion({tipo_:TIPO_DATO.NUMBER,valor_:1});
          }
          return new Expresion({tipo_:TIPO_DATO.NUMBER,valor_:0});
        }else if(valor1.tipo==4){
          //Si es una etiqeuta se convierte a number su valor
          let ejecucionAux=<Ejecucion>valor1.valor;
          if (ejecucionAux.consolaSalidaXPATH.length>0){
            ...
            if (!isNaN(numberValor)){
              return new Expresion({tipo_:TIPO_DATO.NUMBER,valor_:numberValor});
            }
          }
        }
        let resultado=<number>valor1.valor;
        if (!isNaN(resultado)){
            return new Expresion({tipo_:TIPO_DATO.NUMBER,valor_:resultado});
        }
      }
      Errores.getInstance().push(
        new Error({
      ...
      //Si no es error, se ejecuta
        if(valor1.tipo==4){
          ...
          let a1 = <Ejecucion>valor1.valor;
            ...
            return new Expresion({tipo_:TIPO_DATO.STRING,valor_:txtRes2});
        }
        let resultado=<string>valor1.valor;
        let mayus=resultado.toString().toLowerCase();
          return new Expresion({tipo_:TIPO_DATO.STRING,valor_:mayus});
      }
      Errores.getInstance().push(
        new Error({
          tipo: "semantico",
    ...
    //sub1
          ...
          //Si no es error, se ejecuta
            ...
            let txtResultado=resultado.toString().substring(valor2.valor-1);
              return new Expresion({tipo_:TIPO_DATO.STRING,valor_:txtResultado});
          }
          Errores.getInstance().push(
            new Error({
              tipo: "semantico",
  ...
  */
    ...
    ///DOS PUNTOOOOOOOOOOOOOOOOS
      ...
      recorrerXquery(nodo: any, entorno_: Entorno): void{
        if (this.soyNodo('INICIO', nodo)) {
          if (entorno_==null){
            let nuevoEntorno=new Entorno(null);
            this.recorrerXquery(nodo.hijos[0],nuevoEntorno);
                Entornos.getInstance().push(nuevoEntorno);
          }
          else{
      ...
      ejecutarDeclaracion(nodo: any, entorno_:Entorno): void{
        let identificador=nodo.hijos[0].toString();
        ...
        let nuevoSimbolo=new Simbolo({identificador_:identificador,tipo_:resultado.tipo,valor_:resultado.valor});
        entorno_.insertarVariable(nuevoSimbolo,nodo.linea);
      }

      ejecutarDeclaracion2(nodo: any, entorno_:Entorno): void{
        let identificador=nodo.hijos[0].toString();
      ...
      ejecutarReasignacion(nodo: any, entorno_:Entorno): void{
        let identificador=nodo.hijos[0].toString();
        let resultado=this.resolverExpresion(nodo.hijos[1],entorno_);
        if (resultado.tipo==0){
          //Si el resultado es tipo 0 (error), mostrar error
      ...
      ejecutarSalida(nodo: any,entorno_:Entorno): void{
        //let texto="";
      ...
      ejecutarDeclaracionFuncion(nodo: any, entorno_:Entorno): void{
        let identificador=nodo.hijos[1].toString();
        let clave:string=identificador+"$";
        ...
        let nuevoEntorno=new Entorno(entorno_);
        ...
        let nuevaFuncion=new Funcion({identificador_:clave,tipo_:tipoFuncion,L_params_:l_params,L_ins_:bloque,entorno_:nuevoEntorno,valor_:valorInicial});
      ...
      obtenerTipo(cadena:string):TIPO_DATO{
        ...
        return TIPO_DATO.ERROR;
      }

      //Valores iniciales del return de la funcion
      //0,"",false,null
    valorInicialFuncion(cadena:string):any{
      if (cadena.toLowerCase()=="decimal"){
        return 0;
      }else if (cadena.toLowerCase()=="integer"){
        return 0;
    ...
    llamarFuncion(nodo: any,entorno:Entorno): Expresion{
      ...
      let identificador:string=nodo.hijos[0].toString();
      let clave:string=identificador+"$";
      ...
      if (funcion1!=null) {
        ...
        let entornoAux=new Entorno(funcion1.entorno);
        ...
        if (nodoBloque.hijos.length>0){
          this.recorrerFuncion(nodoBloque,entornoAux,funcion1);
            ...
            Entornos.getInstance().push(entornoAux);
        return new Expresion({tipo_:funcion1.tipo,valor_:funcion1.valor});
      }

      return new Expresion({tipo_:TIPO_DATO.ERROR,valor_:"ErrorExpresionFuncion"});
    }

    recorrerFuncion(nodo: any,entorno:Entorno,funcion_:Funcion):void{
    ...
    ejecutarIf(nodo:any,entorno:Entorno,funcion:Funcion):void{
      let L_cond= nodo.hijos[0];
      let cantCondiciones=L_cond.hijos.length;
      let tieneElse:boolean=false;
      ...
      if (tieneElse){
        for (let i=0;i<L_cond.hijos.length-1;i++){
          ...
          /* la condicion viene asi
                ...
                E       Return
          */
          let expresion1:Expresion=this.resolverExpresion(condicionActual.hijos[0],entorno);
          if (expresion1.tipo!=3){
    ...
    ejecutarFOR1(nodo_:any,entorno_:Entorno):void{
      //nodo[0]=id
      let identificador=nodo_.hijos[0].hijos[0].toString();
      
      //nodo[1]=Exp
      let valor=this.resolverExpresion(nodo_.hijos[1].hijos[0],entorno_);
      let nuevoSimbolo=new Simbolo({identificador_:identificador,tipo_:valor.tipo,valor_:valor.valor});
    ...
    ejecutarReturn(nodo: any,entorno_:Entorno): void{
