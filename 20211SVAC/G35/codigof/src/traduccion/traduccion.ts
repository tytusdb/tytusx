...
import { Expresion } from '../ejecucion/Expresion/Expresion'
import { TIPO_DATO } from '../ejecucion/Expresion/Tipo';
import { Etiqueta } from '../ejecucion/etiqueta';
import { Atributo } from '../ejecucion/atributo';
import temporal from './temporal';
import { Error } from '../arbol/error';
import { Errores } from '../arbol/errores';
import metodoC3D from './metodoC3D';
...
export class Traduccion {
  ...
  arrayMetodosC3D = new Array<string>();
  temporalPivote: string;
  simboloTemporalPivote: string;
  temporalPivote2: string;
  operacionSuprema: string;
  contadorOperacionSuprema: number;
  //TRADUCCION NUEVA
  temporalGlobal: temporal;
  codigoTemporal: string;
  codigoTemporalMetodos: string;
  heap: number[];
  stack: number[];
  P: number = 0;
  H: number = 0;
  ...
  //OBJETOS XQUERY07
  raizXQUERY: object;
  contadorStackXquery: number;
  arrayAuxiliarXquery = new Array<Etiqueta>();
  ListaMetodosXQUERY = new Map<String, metodoC3D>();
  temporalStringXQUERYenMain = "";
  posicionXQUERY1 = 0;

  miSalidaXQUERT = ""
  //OBJETOS XQUERY
  ArrayEtiquetasLineal = new Array<Etiqueta>();

  //TRADUCCION NUEVA

  constructor(raiz: Object, raizXML: Object, raiz2: Object, raizxq:Object) {
    ...
    //XQUERY08
    const eje = new Ejecucion(raiz, raizXML, raizxq);
    eje.ejecutar();
  ...
  */
  ...
  recorrerXML(nodo: any): any {
    ...
    if (this.soyNodo('OBJETO', nodo)) {
      ...
      let NuevaEtiqueta = new Etiqueta(id, atributos, hijoEtiqueta, textoo, pos);
  ...
  recorrerXquery(nodo:any, x:Traduccion){
    ...
    if(this.soyNodo('DECLARAR_FUNCION', nodo)){
      ...
      //Creamos un metodo
      let Metodo = new metodoC3D(NumeroParametros, nodo.linea);
...
resolverExpresion(nodo: any): Expresion{
  ...
  //igual que
    ...
    if (expresion1.tipo == 4) {
      ...
      let a1 = <Traduccion>expresion1.valor;
  ...
  return new Expresion({tipo_:TIPO_DATO.ERROR,valor_:"0"});
}//FIN RESOLVER EXP
...
return;
  ...
  */
  traducirDeclaracionVariable(nodo: any) {
  ...
  ContadorParametrosXQUERY(nodo:any){
  ...
  GuardaParametrosXQUERY(nodo:any, posicion:number){
  ...
  GeneraLlamadaMetodo(nombre:string){
  ...
  traducirFor(nodo:any,tipo:number)
  {
 ...
 //console.log(nodo.hijos[1].toString());
  ...
  auxiliarFor() {
   ...
   traducirFor1(nodo:any,tipo:number)
   {
          //Primero resolvemos la consulta de xpath que lleva 
      //XQUERY07
      let instanciaResolverExpresion = new Traduccion(nodo.hijos[1].hijos[0],this.raizXML,null, null);
