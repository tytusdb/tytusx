...
export class Or extends Expression {
  ...
  public traducir(symbolTable: SymbolTable) {
    ...
    this.etiqueta_verdadera = this.childleft.etiqueta_verdadera;
    this.etiqueta_falsa = this.childright.etiqueta_falsa;
    for (let i: number = 0; i < this.childright.etiqueta_verdadera.length; i++) {
      this.etiqueta_verdadera.push(this.childright.etiqueta_verdadera[i]);
    }
  }
