...
export class UpperCase extends Expression {
    ...
    if (Array.isArray(result)) {
      ...
      result.forEach((temp) => {
        if (temp instanceof SymbolXml) {
          this.Upper(temp, coincidencia, symbolTable);
        } else {
          temp && coincidencia.push(temp.toString().toUpperCase());
        }
      });
      this.value = coincidencia;
    } else {
      this.value = result?.toString().toUpperCase();
    }
    return this.value;
  }
  ...
  public graphCST(str: Array<string>, count: Graph): number {
    ...
    str.push(`
        node${NUMID}[label="UPPERCASE",
        fillcolor="LightBlue", style ="filled", shape="box"];
        node${count.getContador()}[label="upper-case",
        fillcolor="LightBlue", style ="filled", shape="box"];
  ...
  public Upper(
    ...
    if (current.node.value !== undefined)
      coincidences.push(current.node.value.toUpperCase());
    ...
    for (const iterator of currentSymbols) {
      this.Upper(iterator, coincidences, symbolTable);
    }
  }
  public traducir(symbolTable: SymbolTable) {
    ...
    Globals3d.str_codigo3d.setValor("upperCase_macano();");
