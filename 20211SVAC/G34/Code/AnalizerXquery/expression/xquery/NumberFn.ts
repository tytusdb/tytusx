...
export class NumberFn extends Expression {
  ...
  public interpret(
    ...
    if (Array.isArray(result)) {
      const coincidencia: Array<number> = [];
      result.forEach((temp) => {
        if (temp instanceof SymbolXml) {
          this.Nmb(temp, coincidencia, symbolTable);
        } else {
  ...
  public Nmb(
    ...
    for (const iterator of currentSymbols) {
      this.Nmb(iterator, coincidences, symbolTable);
    }
  }
  ...
  public graphCST(str: Array<string>, count: Graph): number {
    ...
    str.push(`
        node${NUMID}[label="NUMBER",
        fillcolor="LightBlue", style ="filled", shape="box"];
        node${count.getContador()}[label="number",
        fillcolor="LightBlue", style ="filled", shape="box"];
