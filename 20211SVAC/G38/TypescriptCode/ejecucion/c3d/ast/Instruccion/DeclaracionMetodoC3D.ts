class DeclaracionMetodoC3D extends Codigo3d{
    ...
    private instrucciones: Codigo3d[];

    constructor(nombre: string, instrucciones: Codigo3d[],linea: number, columna: number) {
        super(linea, columna);
        this.nombre = nombre;
    ...
    regla1(etiqueta:string,indiceActual:number){
        ...
        if(marcador != -1){
            ...
            var opt = new FilaOptimizacion("1",codigoEliminado,""
                                                ,this.instrucciones[indiceActual].linea,this.instrucciones[indiceActual].columna);
            ReporteOptimizacion.AgregarFilaOptimizacion(opt);
        }
    }
    regla2(instruccion:SaltoCondicionalC3D,indiceActual:number){
       /* if ... goto etqV;
        * goto etgF;
        * etqV:
        *
        ...
        * */
        ...
        if(this.instrucciones[i+1] instanceof SaltoC3D && this.instrucciones[i+2] instanceof EtiquetaC3D &&
           !this.instrucciones[i+1].eliminado && !this.instrucciones[i+2].eliminado){
            if(instruccion.etiquetaSalto == (<EtiquetaC3D>this.instrucciones[i+2]).etiqueta){
                this.instrucciones[i+1].eliminado = true;
                this.instrucciones[i+2].eliminado = true;
                var opt = new FilaOptimizacion("2",
                                  instruccion.toString()+this.instrucciones[i+1].toString() + this.instrucciones[i+2].toString(),
                                  "",instruccion.linea,instruccion.columna);
                instruccion.etiquetaSalto = (<SaltoC3D>this.instrucciones[i+1]).etiquetaSalto;
