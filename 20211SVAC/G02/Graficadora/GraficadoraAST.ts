...
var cadenaASTXML: string;
var contadorASTXML: number;
function procesarASTXML() {
    ...
    if (instruccionesXML != null && instruccionesXML.length > 0) {
        var nodoPadre: string = "nodo0";                
        contadorASTXML++;
        instruccionesXML.forEach((objetoHijo: Objeto) => {
            if (objetoHijo instanceof Objeto) {
                var nodoHijo: string = "nodo" + contadorASTXML;
...
function recorrerASTXML(element: Objeto, nodoPadre: string) {

    if (element instanceof Objeto) {

        if (element.listaObjetos != null && element.listaObjetos.length > 0) {
            element.listaObjetos.forEach((objetoHijo: Objeto) => {
                if (objetoHijo instanceof Objeto) {
                    var nombreHijo: string = "nodo" + contadorASTXML;
                    ...
                    if(objetoHijo.texto != null && objetoHijo.texto != ""){
                        var nombreNieto: string = "nodo" + contadorASTXML;
