...
import { Instruccion3D, TipoInstruccion3D } from "./Instrucciones3D/Instruccion3D";
import { Representacion3D } from "./Instrucciones3D/Representacion3D";

export class Optimizer{
    reporte: Array<Optimizacion>
    constructor(){
        this.reporte = [];

    }
    aplicar(listaInstrucciones: Array<Instruccion3D>, reporte: Array<Optimizacion>): Array<Instruccion3D>{
    ...
    instruccionesToString(arrayInst: Array<Instruccion3D>): string{
        let cadena = "";
        arrayInst.forEach((inst: Instruccion3D) => {
            cadena += inst.getCodigo3D()+"\n";
        });
        return cadena;
    }    
    
    agregarReporte(codigoNuevo: Array<Instruccion3D>, codigoEliminado: Array<Instruccion3D>, numRegla: ReglaOptimizacion){
    ...
    /*          REGLA 1 
        ...
        T3 = T1+T3;                         T3 = T1+T3;
    */
    regla1(listaInstrucciones: Array<Instruccion3D> | null): Array<Instruccion3D> {
    ...
    /*          REGLA 2
        ...
        <instr L2>
    */
    regla2(listaInstrucciones: Array<Instruccion3D>){
                ...
                if(instruccion instanceof If3D){
                    ...
                    if(i + 1 < listaInstrucciones.length){
                        ...
                        if(sigInst instanceof Goto3D){
                            ...
                            let auxReporte: Array<Instruccion3D> = [];
    ...
    */    
    regla3_y_regla4(listaInstrucciones: Array<Instruccion3D>): Array<Instruccion3D>{
    ...
    */    
    ...
    regla5(listaInstrucciones: Array<Instruccion3D>): Array<Instruccion3D>{
            ...
            if(instruccion instanceof Asignacion3D){
                ...
                if(asignacion instanceof Primitiva3D){
                    if(asignacion.getTipoPrim3D() === TipoPrim3D.IDENTIFIER){
                        ...
                        let n = i+1;
                        let auxInst;
                        let auxiliarInstr: Array<Instruccion3D> = [];
    ...
    reglasAlgebraicas(listaInstrucciones: Array<Instruccion3D>): Array<Instruccion3D>{
    ...
    sonConstantes(izq: Primitiva3D, der: Primitiva3D | null): boolean{
        if(izq.getTipoPrim3D() == TipoPrim3D.IDENTIFIER){
            return false;
        }
        if(der != null){
